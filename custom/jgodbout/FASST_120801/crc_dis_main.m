...
%      CRC_DIS_MAIN, by itself, creates a new CRC_DIS_MAIN or raises the existing
%      singleton*.
%
...
% $Id: crc_dis_main.m 257 2010-10-15 13:09:26Z christophe $
...
if isfield(varargin{1},'multcomp') %if comparing multiple files
    ...
    set(handles.slider1,...
        ...
        'Min',0)
    try
        %         set(handles.slider1,'SliderStep',[(handles.winsize*fsample(handles.Dmeg{1}))/nsamples(handles.Dmeg{1}) 0.1])
    ...
    set(handles.Chanslider,...
        ...
        'Value',1,...
        'SliderStep', [Nchdisp/length(handles.index) Nchdisp/length(handles.index)])
    %handles popupmenustring using all channels of file
...
if handles.multcomp
    ...
    i=min(length(handles.index),NbreChandisp);
end
...
Nchdisp=str2double(get(handles.NbreChan,'String'));
if ~isnan(Nchdisp) && length(Nchdisp)==1
    set(handles.NbreChan,'String', num2str(min(Nchdisp, length(handles.index))))
    Nchdisp=str2double(get(handles.NbreChan,'String'));
...
for i=1:size(sc,2)
    theosz=ceil(nsamples(handles.Dmeg{1})/fsample(handles.Dmeg{1})/sc{3,i});
...
%% Call for visual comparison of hypnograms
% --------------------------------------------------------------------
...
handles.popcmp(1) = uicontrol(... % List of available scorer
    'Parent', z, ...
    'Units','normalized',...
...
handles.popcmp(2) = uicontrol(... % List of available scorer
    'Parent', z, ...
    'Units','normalized',...
...
hUpdateButton = uicontrol(... % Button for merging
    'Parent', z, ...
    'Units','normalized',...
    'HandleVisibility','callback', ...
...
% corresonding parts
% plothypnocompare(handles.popcmp(1),[],handles,Mainhandle)
if size(handles.Dmeg{1}.CRC.score{1,1})==size(handles.Dmeg{1}.CRC.score{1,2})
    handles.match = handles.Dmeg{1}.CRC.score{1,1} - ...
        handles.Dmeg{1}.CRC.score{1,2};
...
if size(handles.Dmeg{1}.CRC.score{1,Val1})==size(handles.Dmeg{1}.CRC.score{1,Val2})
    ...
    handles.match = handles.Dmeg{1}.CRC.score{1,Val1} - ...
        handles.Dmeg{1}.CRC.score{1,Val2};
    matchvect = handles.match;
    matchvect(find(handles.match ~= 0)) = 7;
...
if handles.Dmeg{1}.CRC.score{3,Val1}==handles.Dmeg{1}.CRC.score{3,Val2}
    ...
    handles.Dmeg{1}.CRC.score{4,end}(1) = min(...
        handles.Dmeg{1}.CRC.score{4,Val1}(1), ...
        handles.Dmeg{1}.CRC.score{4,Val2}(1));
    handles.Dmeg{1}.CRC.score{4,end}(2) = max( ...
        handles.Dmeg{1}.CRC.score{4,Val1}(2), ...
...
if handles.adddeb(handles.currentscore) == 1
    if ~isempty(handles.score{5,handles.currentscore})
        if sum(and(timedbtart>handles.score{5,handles.currentscore}(:,1),...
            ...
            handles.score{5,handles.currentscore}= ...
                [handles.score{5,handles.currentscore}; timedbtart timedbtart];
            ...
            fact=min(str2double(get(handles.NbreChan,'String')),length(handles.index));
...
if Mouse(1,2) > 0 && handles.scoring
    ...
    hold on
    handles.cursor=plot(slidval+handles.winsize/2,8,'v','Color',[0.2 0.2 0.2],'LineWidth',2.5);
    hold on
...
for i=1:maxi
    ...
    if ~nosign  %if multiple comparison and no signal in the considered file
        ...
        for j=1:maxj
            ...
            testcond = chanlabels(handles.Dmeg{i},index(j));
            if handles.multcomp
            ...
            switch  selchan
                ...
                case    'MEAN OF REF'
                    ...
                    scnddata = handles.Dmeg{i}(index(j),toshow) - ...
                        handles.Dmeg{i}(ref2idx,toshow);
                ...
                case    'BIPOLAR'
                    ...
                    if ~isempty(index3)
                        bipolar = handles.Dmeg{i}(index(j),toshow) - ...
                            handles.Dmeg{i}(index3,toshow);
                        plt=plot(temps,factscale+(bipolar)/scal,'Color',[0.2 0.9 0.5]);
                    else
                        plt=plot(temps,factscale+(handles.Dmeg{i}(index(j),toshow))/scal, ...
                            'Color',[0.2 0.5 0.9]);
                    end
                ...
                otherwise
                    [dumb1,index3] = ...
                        intersect(upper(chanlabels(handles.Dmeg{i})),selchan);
                    basedata = handles.Dmeg{i}(index(j),toshow);
                    toplotdat = basedata - ...
                        handles.Dmeg{i}(index3,toshow);
                    plt=plot(temps,factscale+(toplotdat)/scal);
...
if handles.scoring
    ...
    if ~isempty(handles.score{5,handles.currentscore})
        tdebs=str2double(get(handles.currenttime,'String'))-handles.winsize/2;
        tfins=str2double(get(handles.currenttime,'String'))+handles.winsize/2;
        startart=find(and(handles.score{5,handles.currentscore}(:,1)>tdebs,...
            handles.score{5,handles.currentscore}(:,1)<tfins));
        endart=find(and(handles.score{5,handles.currentscore}(:,2)>tdebs,...
            handles.score{5,handles.currentscore}(:,2)<tfins));
...
if ~handles.multcomp
    ...
    if Nev_dis % do all this if there are triggers to be displayed !
        ...
        plot(int,0.5*ones(1,length(int))*handles.scale/50*NbreChandisp, ...
            'k^','LineWidth',2.5)
        %chose between the diplay of type or of value
        fmric=0;
