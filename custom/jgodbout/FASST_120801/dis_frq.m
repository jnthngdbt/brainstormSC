...
% $Id: dis_frq.m 456 2012-06-21 14:11:18Z christophe $
...
[pth name]= fileparts(handles.file);
...
% Check if the frq file is in the right section
...
handles.figz = 0;
handles.currentscore=get(handles.scorertxt,'Value')+1;
handles.addpoi = 1;
...
if isfield(handles.Dmeg,'CRC')
    if isfield(handles.Dmeg.CRC,'score')
        ...
        crc_plothypno(handles.Hypnogra,...
            handles.Dmeg.CRC.score{4,1},handles,...
            handles.Dmeg.CRC.score{3,1},...
            handles.Dmeg.CRC.score{1,1});
...
%Plot the spectrogram
toshow=toshow(1:length(toshow)-1);
picstep = max(1,floor(size(handles.Dmeg.CRC.pwrspect.frqdata,3)/handles.maxpix));
toshowpic = 1:picstep:size(handles.Dmeg.CRC.pwrspect.frqdata,3);
Spectr(:,:)=handles.Dmeg.CRC.pwrspect.frqdata(handles.spectrochan,2:end-1,toshowpic);
hspec=imagesc(log(Spectr));
if ~handles.export
...
current=get(handles.figure1,'CurrentAxes');
set(handles.figure1,'CurrentAxes',handles.Hypnogra)
...
set(handles.figure1,'CurrentAxes',current)
...
switch  stage
    ...
    case 8
        score = find(handles.Dmeg.CRC.score{1,numscore} > 1 & ...
            handles.Dmeg.CRC.score{1,numscore} < 5 );
    case 7
        score = find(handles.Dmeg.CRC.score{1,numscore} > 2 & ...
            handles.Dmeg.CRC.score{1,numscore} < 5 );
end
...
try
    ...
    semilogy(handles.Dmeg.CRC.pwrspect.frqbins, mean_power_spectrum)

    % bit to save the values in a mat file
    [frqpath frqname]=fileparts(handles.file);
    stagestr2 = stagestr;
    stagestr2(strfind(stagestr2,' '))=[];
    stagestr2(strfind(stagestr2,'('))=[];
    stagestr2(strfind(stagestr2,')'))=[];
    frqname= fullfile(frqpath,['frq_',frqname,'_',selchan,'_',stagestr2]);
    save(frqname,'mean_power_spectrum')
